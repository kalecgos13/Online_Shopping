...
public class Year_wise_user_report extends Thread {

    ServletContext c;

    public Year_wise_user_report(ServletContext c) {
        this.c = c;
    }
    ...
    public void run() {
        ...
        try {
            ...
            for (int i = 0; i < 12; i++) {
                String query = "select sum(user_qty) from tbl_order_detail,tbl_sub_cat,tbl_category,tbl_product,tbl_order where tbl_order_detail.p_id = tbl_product.p_id and  tbl_order_detail.o_id = tbl_order.o_id and tbl_product.sub_id = tbl_sub_cat.sub_id and tbl_sub_cat.c_id = tbl_category.c_id and tbl_order.order_date between '" + year + "-01-01' and '" + year + "-12-31'";
                Statement st1 = cnn.createStatement();
                ResultSet rs1 = st1.executeQuery(query);
                while (rs1.next()) {

                    int total_saling = rs1.getInt(1);
                    if (total_saling > 0) {
            ...
            try {
                ...
                final File file1 = new File(c.getRealPath(".") + "/images/productImages/yearwise.png");
                ChartUtilities.saveChartAsPNG(file1, barChart, 700, 500, info);
            } catch (Exception e) {
